...
def sample_images_dict():
    ...
    return {
        ...
        'satellite': [
            'data/test_bundle/satellite/satellite/latest.png'
        ],
        'sst_charts': [
            'data/test_bundle/charts/sst_anomaly.png'
        ]
    }
        ...
        'swell_events': [
            {
                ...
                'quality_flag': 'valid',
                'primary_components': [
                    {'height': 2.0, 'period': 14.0, 'quality_flag': 'valid'}
                ],
            ...
            },
            {
                ...
                'primary_components': [
                    {'height': 1.5, 'period': 15.0, 'quality_flag': 'excluded'}
                ],
                'secondary_components': []
            },
            {
...
def test_prepare_forecast_data_filters_excluded_components(data_manager, mock_logger):
    ...
    assert len(result['swell_events'][0]['primary_components']) == 1, "Should filter excluded primary component"
    assert result['swell_events'][0]['primary_components'][0]['direction'] == 315.0, "Should keep valid component"
        ...
        'swell_events': [
            {
                ...
                'quality_flag': 'valid',
                'primary_components': [
                    {'height': 1.5, 'quality_flag': 'excluded'}
                ],
                'secondary_components': [
                    {'height': 1.0, 'quality_flag': 'excluded'}
                ]
            }
        ]
    }
...
def test_filter_quality_data_preserves_original(data_manager, sample_fused_data):
    ...
    # Assert
    assert len(sample_fused_data['buoy_observations']) == original_buoy_count, "Original should be unchanged"
    assert len(sample_fused_data['swell_events']) == original_event_count, "Original should be unchanged"
...
def test_select_critical_images_prioritization(data_manager, sample_images_dict):
    ...
    # Assert
    assert len(result) == 9, "Should select 9 total images (4 pressure + 3 wave + 1 satellite + 1 sst)"
...
def test_select_critical_images_max_limit_enforcement(data_manager, sample_images_dict):
    ...
    assert all(img['type'] == 'pressure_chart' for img in result[:4]), "Should prioritize pressure charts"
...
def test_get_seasonal_context_winter():
    ...
    assert result['seasonal_patterns']['north_shore']['quality'] == 'High', "North Shore should be prime in winter"
    assert result['seasonal_patterns']['south_shore']['quality'] == 'Low', "South Shore should be low in winter"
...
def test_get_seasonal_context_summer():
    ...
    assert result['seasonal_patterns']['south_shore']['quality'] == 'High', "South Shore should be prime in summer"
    assert result['seasonal_patterns']['north_shore']['quality'] == 'Low', "North Shore should be low in summer"
...
def test_get_seasonal_context_spring():
    ...
    assert 'transition' in result['seasonal_patterns']['north_shore']['typical_conditions'].lower(), "Should indicate transition"
...
def test_estimate_tokens_basic(data_manager):
    ...
    forecast_data = {
        'swell_events': [
            {'event_id': 'event1', 'hawaii_scale': 6.0, 'primary_components': []}
        ],
        'shore_data': {
            'north_shore': {'swell_events': []}
        },
        'images': {
...
def test_estimate_tokens_respects_image_limits(data_manager, mock_logger):
    ...
    forecast_data = {
        ...
        'shore_data': {},
        'images': {
            'pressure_charts': ['p1.png', 'p2.png', 'p3.png', 'p4.png', 'p5.png'],  # 5 but only 4 counted
            'wave_models': ['w1.png', 'w2.png', 'w3.png', 'w4.png', 'w5.png', 'w6.png'],  # 6 but only 4 counted
