...
def mock_config_no_openai_key():
    ...
    def get_side_effect(section, key, fallback=None):
        config_map = {("openai", "api_key"): None, ("openai", "max_tokens"): 2000}  # No key
        return config_map.get((section, key), fallback)
...
def sample_pressure_data(tmp_path):
    ...
    return {
        ...
        "metadata": {"chart_times": ["2025-10-09T00:00:00Z"], "region": "North Pacific"},
    }
...
@pytest.mark.asyncio
async def test_buoy_analyst_uses_model_in_api_call(
    mock_config_tiered, mock_engine, sample_buoy_data
):
...
async def test_pressure_analyst_uses_custom_model_in_api_call(
    mock_config_tiered, mock_engine, sample_pressure_data
):
...
async def test_buoy_analyst_works_without_openai_key(
    mock_config_no_openai_key, mock_engine, sample_buoy_data
):
