...
class TestCallOpenAIAPITextMode:
    ...
    async def test_call_openai_api_respects_max_tokens(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai_class:
            ...
            assert call_kwargs['max_completion_tokens'] == max_tokens, "Should match configured value"
...
class TestCallOpenAIAPIMultimodalMode:
    ...
    async def test_call_openai_api_converts_local_image_to_data_url(self, tmp_path):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai_class:
            with patch.object(client, '_convert_image_to_data_url', return_value='data:image/png;base64,test'):
                mock_response = Mock()
                mock_response.choices = [Mock(message=Mock(content='Analysis'))]
                mock_response.usage = Mock(prompt_tokens=200, completion_tokens=100)
                ...
                # Act
                await client.call_openai_api('system', 'user', image_urls=[f'data/{image_path.name}'])
    ...
    async def test_call_openai_api_falls_back_on_image_error(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai_class:
            with patch.object(client, '_convert_image_to_data_url', side_effect=FileNotFoundError("Not found")):
                mock_response = Mock()
                ...
                result = await client.call_openai_api('system', 'user', image_urls=['data/missing.png'])
...
class TestTrackUsage:
    ...
    async def test_track_usage_gpt5_nano_pricing(self):
        ...
        assert abs(client.total_cost - expected_cost) < 1e-10, "Cost should match gpt-5-nano pricing"
    ...
    async def test_track_usage_gpt5_mini_pricing(self):
        ...
        assert abs(client.total_cost - expected_cost) < 1e-10, "Cost should match gpt-5-mini pricing"
...
class TestThreadSafety:
    ...
    async def test_concurrent_cost_tracking_maintains_accuracy(self):
        ...
        usage_objects = [
            Mock(prompt_tokens=100, completion_tokens=50) for _ in range(10)
        ]
        ...
        assert abs(client.total_cost - expected_cost) < 1e-10, "Should calculate total cost correctly"
    ...
    async def test_concurrent_api_calls_dont_corrupt_state(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai_class:
            ...
            tasks = [
                client.call_openai_api('system', f'user{i}')
                for i in range(5)
            ]
...
class TestEdgeCases:
    ...
    async def test_temperature_parameter_handling(self):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai_class:
            ...
            # Assert
            assert 'temperature' in call_with_temp_kwargs, "Should include temperature when specified"
            assert call_with_temp_kwargs['temperature'] == 0.7, "Should match specified value"
            assert 'temperature' not in call_without_temp_kwargs, "Should not include temperature when None"
