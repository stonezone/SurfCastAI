...
class TestGroupVelocityCalculation:
    ...
    def test_12_second_period_velocity(self):
        ...
        assert velocity_knots == pytest.approx(
            18.2, abs=0.5
    ...
    def test_zero_period_returns_zero(self):
        ...
        assert velocity == 0.0, "Zero period should yield zero velocity"
    ...
    def test_negative_period_yields_negative_velocity(self):
        ...
        assert velocity < 0, "Negative period should yield negative velocity"

    @pytest.mark.parametrize(
        "period,expected_velocity_ms",
        [
...
class TestHaversineDistance:
    ...
    def test_hawaii_to_kamchatka_distance(self):
        ...
        """
        calc = SwellPropagationCalculator()
        distance = calc.haversine_distance(45.0, 155.0, HAWAII_LAT, HAWAII_LON)  # Kamchatka
    ...
    def test_hawaii_to_new_zealand_distance(self):
        ...
        """
        calc = SwellPropagationCalculator()
        distance = calc.haversine_distance(-40.0, 174.0, HAWAII_LAT, HAWAII_LON)  # New Zealand
    ...
    def test_hawaii_to_alaska_distance(self):
        ...
        """
        calc = SwellPropagationCalculator()
        distance = calc.haversine_distance(55.0, -160.0, HAWAII_LAT, HAWAII_LON)  # Alaska
    ...
    def test_zero_distance_same_location(self):
        ...
        """
        calc = SwellPropagationCalculator()
        distance = calc.haversine_distance(HAWAII_LAT, HAWAII_LON, HAWAII_LAT, HAWAII_LON)
    ...
    def test_equator_to_equator_distance(self):
        ...
        distance = calc.haversine_distance(
            0.0, 0.0, 0.0, 10.0  # Equator, Prime Meridian  # Equator, 10°E
        )

        # 10° × 60 nm/degree ≈ 600 nm
        assert 580 <= distance <= 620, f"10° along equator should be ~600 nm, got {distance:.0f} nm"
    ...
    def test_dateline_crossing(self):
        ...
        distance = calc.haversine_distance(35.7, 140.0, HAWAII_LAT, HAWAII_LON)  # Tokyo
    ...
    def test_antipodal_points_maximum_distance(self):
        ...
        distance = calc.haversine_distance(HAWAII_LAT, HAWAII_LON, -HAWAII_LAT, -HAWAII_LON + 180)

        # Should be close to maximum possible distance (>8000 nm)
        assert distance > 8000, f"Antipodal distance should be >8,000 nm, got {distance:.0f} nm"
...
class TestTravelTimeCalculation:
    ...
    def test_zero_distance_zero_travel_time(self):
        ...
        calc = SwellPropagationCalculator()
        travel_hours, distance_nm, velocity_kt = calc.calculate_travel_time(
            HAWAII_LAT, HAWAII_LON, 12.0  # Source at Hawaii  # 12s period
        )

        assert distance_nm == pytest.approx(0.0, abs=0.01), "Zero distance expected"

        assert travel_hours == pytest.approx(0.0, abs=0.01), "Zero travel time expected"
...
class TestArrivalCalculation:
    ...
    def test_kamchatka_arrival_datetime(self):
        ...
        arrival_time, details = calc.calculate_arrival(
            45.0, 155.0, 16.0, generation_time  # Kamchatka  # 16s long-period swell
        )

        # Verify arrival is in the future
        assert arrival_time > generation_time, "Arrival should be after generation"

        # Verify arrival is ~3-5 days later (16s period = ~24 kt velocity)
        delta = arrival_time - generation_time
        assert 3.0 <= delta.days <= 5.0, f"Arrival should be 3-5 days later, got {delta.days} days"
    ...
    def test_new_zealand_arrival_datetime(self):
        ...
        arrival_time, details = calc.calculate_arrival(
            -40.0, 174.0, 15.0, generation_time  # New Zealand  # 15s period
        )

        # Verify arrival timing (~6-8 days for long distance)
        delta = arrival_time - generation_time
        assert 6.0 <= delta.days <= 8.0, f"Arrival should be 6-8 days later, got {delta.days} days"
    ...
    def test_immediate_arrival_zero_distance(self):
        ...
        arrival_time, details = calc.calculate_arrival(
            HAWAII_LAT, HAWAII_LON, 12.0, generation_time  # Source at Hawaii
        )

        # Arrival should be essentially immediate
        delta = arrival_time - generation_time
        assert (
            delta.total_seconds() < 1.0
...
class TestPeriodEstimation:
    ...
    def test_storm_force_winds_base_period(self):
        ...
        assert 8.0 <= period <= 20.0, f"Period should be in 8-20s range, got {period:.1f}s"
    ...
    def test_moderate_winds_base_period(self):
        ...
        assert period >= 8.0, f"Period should be >=8s (minimum), got {period:.1f}s"
    ...
    def test_large_fetch_increases_period(self):
        ...
        assert (
            long_fetch_period > short_fetch_period
        ...
        assert (
            long_fetch_period - short_fetch_period >= 1.0
    ...
    def test_long_duration_increases_period(self):
        ...
        assert (
            long_duration_period > short_duration_period
    ...
    def test_combined_fetch_and_duration_effects(self):
        ...
        enhanced_period = calc.estimate_period_from_storm(
            50.0, fetch_length_nm=800.0, duration_hours=72.0
        )

        assert enhanced_period > base_period, "Enhanced conditions should yield longer period"

        # Should approach upper limit (20s) but not exceed it
        assert (
            enhanced_period <= 20.0
    ...
    def test_period_clamped_to_minimum_8s(self):
        ...
        assert period == 8.0, f"Minimum period should be 8s, got {period:.1f}s"
    ...
    def test_period_clamped_to_maximum_20s(self):
        ...
        period = calc.estimate_period_from_storm(
            100.0, fetch_length_nm=2000.0, duration_hours=120.0
        )

        assert period == 20.0, f"Maximum period should be 20s, got {period:.1f}s"

    @pytest.mark.parametrize(
        "wind_speed,expected_range",
        [
...
class TestIntegrationScenarios:
    ...
    def test_pat_caldwell_kamchatka_forecast(self):
        ...
        period = calc.estimate_period_from_storm(
            wind_speed_kt=50.0, fetch_length_nm=500.0, duration_hours=72.0
        )
    ...
    def test_southern_hemisphere_winter_swell(self):
        ...
        period = calc.estimate_period_from_storm(
            wind_speed_kt=60.0, fetch_length_nm=700.0, duration_hours=60.0
        )
        ...
        arrival_time, details = calc.calculate_arrival(-40.0, 174.0, period, generation_time)
    ...
    def test_short_period_local_swell(self):
        ...
        period = calc.estimate_period_from_storm(
            wind_speed_kt=40.0, fetch_length_nm=300.0, duration_hours=36.0
        )
        ...
        arrival_time, details = calc.calculate_arrival(55.0, -160.0, period, generation_time)
