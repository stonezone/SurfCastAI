    ...
    ds = xr.Dataset(
        {
            ...
            "waveTime": (["time"], ["2025-10-15T10:00:00Z", "2025-10-15T11:00:00Z", "2025-10-15T12:00:00Z"]),
            "waveFrequency": (["freq"], np.array([0.04, 0.05, 0.06, 0.07])),
            "waveEnergyDensity": (["time", "freq"], np.array([[1.0, 1.2, 0.8, 0.4], [1.1, 1.3, 0.9, 0.5], [1.2, 1.4, 1.0, 0.6]])),
        },
        attrs={
            "station_name": "Waimea Bay",
            "latitude": 21.65,
            "longitude": -158.07
        }
    )
...
def test_cdip_agent_both_sources_fail(tmp_path):
    ...
    agent.http_client = SimpleNamespace(
        download=AsyncMock(return_value=failed_result)
    )
