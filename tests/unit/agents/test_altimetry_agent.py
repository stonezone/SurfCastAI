...
def test_altimetry_agent_downloads_erddap_png(tmp_path):
    ...
    config.get_data_source_urls.return_value = {
        'altimetry': ['https://upwell.pfeg.noaa.gov/erddap/griddap/nesdisSSH1day.graph?sla[(latest)][(15):(30)][(-165):(-150)]&amp;.draw=surface&amp;.vars=longitude|latitude|sla&amp;.size=800|600&amp;.png']
    }
...
def test_altimetry_agent_processes_zip_payload(tmp_path):
    config = MagicMock()
    config.get_data_source_urls.return_value = {
        'altimetry': ['https://example.com/archive.zip']
    }
...
def test_altimetry_agent_processes_netcdf_gzip(tmp_path):
    config = MagicMock()
    config.get_data_source_urls.return_value = {
        'altimetry': ['https://example.com/pass.nc.gz']
    }
    ...
    assert 'netcdf_summary' in entry or 'netcdf_dimensions' in entry or 'netcdf_summary_error' in entry
