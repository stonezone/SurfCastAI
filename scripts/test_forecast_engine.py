...
def validate_forecast_content(forecast, logger):
    ...
    """
    results = {"success": True, "reasons": [], "metrics": {}}
    ...
    for shore, keywords in shore_keywords.items():
        if shore in forecast:
            ...
            if keyword_ratio < 0.5:  # At least half of the expected keywords should be present
                logger.warning(
                    f"Section '{shore}' may lack specificity (keyword ratio: {keyword_ratio:.2f})"
                )
...
def create_test_swell_forecast():
    ...
    event.primary_components.append(
        SwellComponent(height=3.0, period=15.0, direction=315, confidence=0.9, source="model")
    )

    # Add secondary component
    event.secondary_components.append(
        SwellComponent(height=1.5, period=10.0, direction=300, confidence=0.7, source="model")
    )
    ...
    event2.primary_components.append(
        SwellComponent(height=1.5, period=12.0, direction=190, confidence=0.8, source="model")
    )
    ...
    event3.primary_components.append(
        SwellComponent(height=1.0, period=8.0, direction=70, confidence=0.9, source="buoy")
    )
        ...
        metadata={
            ...
            "tides": {
                "high_tide": [
                    ...
                    2.1,
                ],  # 2.1 feet at 4 hours from now
                "low_tide": [
                    (now + timedelta(hours=10)).strftime("%H:%M"),
                    0.3,
                ],  # 0.3 feet at 10 hours from now
            },
        },
    )
