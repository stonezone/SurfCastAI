...
║  🧠 AI MODEL: """ + f"{self.current_model:<13}" + """     🔮 FORECAST ENGINE: [READY]     ║
║                                                                              ║
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    def show_main_menu(self):
        ...
        menu_items = [
            ("1", "🚀 FULL NEURAL PIPELINE", "Execute complete data collection → processing → forecast → AI analysis", NEON_CYAN),
            ("2", "📡 DATA COLLECTION ONLY", "Gather fresh intel from satellites, buoys, and weather stations", NEON_GREEN),
            ("3", "⚙️  DATA PROCESSING ONLY", "Process existing data bundles through quantum fusion algorithms", NEON_YELLOW),
            ("4", "🔮 FORECAST GENERATION", "Generate AI-powered surf predictions from processed data", NEON_MAGENTA),
            ("5", "🧠 AI ANALYSIS ONLY", "Run GPT analysis on existing forecasts", NEON_ORANGE),
            ("", "─" * 75, "", "dim"),
            ("6", "🎯 GPT MODEL SELECTOR", f"Current: {self.current_model} | Switch AI analysis models", WARNING_AMBER),
            ("7", "📊 BUNDLE MANAGER", "View, analyze, and manage data bundles", NEON_CYAN),
            ("8", "📈 SYSTEM MONITORING", "Live logs, performance metrics, health status", MATRIX_GREEN),
            ("9", "🛠️  DEVELOPER TOOLS", "Benchmarks, tests, demos, and debugging utilities", NEON_YELLOW),
            ("", "─" * 75, "", "dim"),
            ("A", "⚙️  CONFIGURATION", "View and modify system configuration", NEON_GREEN),
            ("B", "📚 HELP & EXAMPLES", "Documentation, usage examples, and tutorials", NEON_MAGENTA),
    ...
    async def run_pipeline_mode(self, mode: str, description: str):
        ...
        try:
            ...
            # Show fancy progress
            console.print(f"[{NEON_GREEN}]📡 Establishing quantum link to data sources...[/{NEON_GREEN}]")
            time.sleep(1)
            
            console.print(f"[{NEON_YELLOW}]⚡ Spinning up neural processors...[/{NEON_YELLOW}]")
            time.sleep(1)
            
            console.print(f"[{NEON_MAGENTA}]🧠 Loading AI model: {self.current_model}[/{NEON_MAGENTA}]")
            time.sleep(1)
            ...
            with Progress(
                ...
                task = progress.add_task(f"[{NEON_GREEN}]Executing {mode} pipeline...[/{NEON_GREEN}]", total=None)
    ...
    def show_pipeline_results(self, results: Dict[str, Any], mode: str):
        ...
        table = Table(title=f"[{NEON_CYAN}]🎯 PIPELINE EXECUTION REPORT - {mode.upper()}[/{NEON_CYAN}]",
                     box=box.DOUBLE_EDGE, border_style=NEON_GREEN)
        ...
        if 'forecast' in results and 'formats' in results['forecast']:
            ...
            for format_name, file_path in formats.items():
                console.print(f"   • [{NEON_YELLOW}]{format_name.upper()}[/{NEON_YELLOW}]: {file_path}")
        ...
        try:
            ...
            if new_model != self.current_model:
                ...
                try:
                    ...
                    console.print(f"[{WARNING_AMBER}]⚠️ Could not update config file: {e}[/{WARNING_AMBER}]")
            else:
    ...
    def show_bundle_manager(self):
        ...
        try:
            ...
            if not bundles:
                console.print(f"[{WARNING_AMBER}]📭 No data bundles found in quantum storage[/{WARNING_AMBER}]")
            else:
                ...
                for i, bundle in enumerate(bundles[:10]):  # Show top 10
                    ...
                    rate_color = NEON_GREEN if success_rate > 80 else WARNING_AMBER if success_rate > 50 else ERROR_RED
    ...
    def show_system_status(self):
        ...
        if SURFCAST_AVAILABLE:
            table.add_row(
                ...
                f"[dim]Neural modules loaded[/dim]"
            )
        else:
            table.add_row(
                f"[{NEON_CYAN}]SURFCAST AI[/{NEON_CYAN}]",
                f"[{ERROR_RED}]❌ OFFLINE[/{ERROR_RED}]",
                f"[dim]Import error detected[/dim]"
            )
        ...
        if self.config:
            ...
            table.add_row(
                ...
                f"[dim]Config file not found[/dim]"
            )
    ...
    def show_developer_tools(self):
        ...
        choice = Prompt.ask(f"[{NEON_GREEN}]Select tool[/{NEON_GREEN}]", choices=["1","2","3","4","5","6","q"], default="q")
    ...
    def run_script(self, script_name: str):
        ...
        try:
            ...
            console.print(f"[{NEON_CYAN}]✅ Script completed with exit code: {result.returncode}[/{NEON_CYAN}]")
    ...
    def run_performance_analysis(self):
        ...
        try:
            ...
            import numpy, pandas, aiohttp, pydantic, openai
    ...
    def show_help(self):
        ...
        console.print("  • [dim]Use 'Forecast Only' mode to generate multiple forecasts from same data[/dim]")
        ...
        if readme_path.exists():
            ...
            try:
                with open(readme_path, 'r') as f:
                    ...
                    if len(content) >= 2000:
                        console.print(f"[dim]... (content truncated)[/dim]")
            except Exception as e:
        ...
        if migration_path.exists():
            ...
            try:
                with open(migration_path, 'r') as f:
                    ...
                    content = "".join(lines)
                    console.print(Panel(content, border_style=NEON_MAGENTA, title="MIGRATION_COMPLETE.md"))
            except Exception as e:
