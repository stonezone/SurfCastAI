...
class SeniorForecaster(BaseSpecialist):
    ...
    def __init__(self, config: Optional[Any] = None, model_name: Optional[str] = None, engine: Optional[Any] = None):
        ...
        if config:
            ...
            self.min_specialists_required = config.getint('forecast', 'require_minimum_specialists', 2)
        else:
            self.openai_api_key = os.environ.get('OPENAI_API_KEY')
            self.max_tokens = 2000
            self.min_specialists_required = 2

    async def analyze(self, data: Union[Dict[str, Any], SeniorForecasterInput]) -> SeniorForecasterOutput:
        """
        ...
        if isinstance(buoy_analysis, BuoyAnalystOutput):
            ...
            buoy_specialist = type('obj', (object,), {
                'confidence': buoy_analysis.get('confidence', 0.0) if buoy_analysis else 0.0,
                'data': buoy_analysis.get('data', {}) if buoy_analysis else {},
                'narrative': buoy_analysis.get('narrative', '') if buoy_analysis else ''
            })() if buoy_analysis else None
        ...
        if isinstance(pressure_analysis, PressureAnalystOutput):
            ...
            # Wrap in a simple object with confidence attribute for compatibility
            pressure_specialist = type('obj', (object,), {
                'confidence': pressure_analysis.get('confidence', 0.0) if pressure_analysis else 0.0,
                'data': pressure_analysis.get('data', {}) if pressure_analysis else {},
                'narrative': pressure_analysis.get('narrative', '') if pressure_analysis else ''
            })() if pressure_analysis else None
        ...
        try:
            ...
            shore_forecasts = self._generate_shore_forecasts(
                buoy_specialist, pressure_specialist, input_dict.get('shore_data', {}),
                input_dict.get('seasonal_context', {})
            )

            # Generate swell breakdown (use wrapped specialists)
            swell_breakdown = self._generate_swell_breakdown(
                buoy_specialist, pressure_specialist
            )
            ...
            narrative = await self._generate_caldwell_narrative(
                buoy_specialist, pressure_specialist,
                contradictions, key_findings, shore_forecasts, swell_breakdown,
                input_dict.get('seasonal_context', {}), input_dict.get('metadata', {})
            )
    ...
    def _identify_contradictions(
        self,
        buoy_analysis: Optional[SpecialistOutput],
        pressure_analysis: Optional[SpecialistOutput]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for swell in pressure_swells:
            ...
            for trend in buoy_trends:
                ...
                if trend_direction is not None and self._directions_match(str(trend_direction), swell_direction):
    ...
    def _find_supporting_swell(
        self,
        trend: Dict[str, Any],
        predicted_swells: List[Dict[str, Any]]
    ...
    def _calculate_specialist_agreement(
        ...
        if agreement_factors:
            ...
            weights = [0.4, 0.4, 0.2] if len(agreement_factors) == 3 else [1.0 / len(agreement_factors)] * len(agreement_factors)
            return sum(f * w for f, w in zip(agreement_factors, weights))
    ...
    def _compare_directions(
        self,
        buoy_directions: List[float],
        pressure_directions: List[float]
    ) -> float:
        """
    ...
    def _compare_trends(
        self,
        buoy_trends: List[Dict[str, Any]],
        pressure_swells: List[Dict[str, Any]]
    ) -> float:
        """
    ...
    def _generate_shore_forecasts(
        ...
        shore_exposures = {
            'north_shore': {'directions': [315, 0, 45], 'primary': True if season == 'winter' else False},
            'south_shore': {'directions': [135, 180, 225], 'primary': True if season == 'summer' else False},
        ...
        for shore_name, exposure in shore_exposures.items():
            ...
            confidence = self._estimate_shore_confidence(relevant_swells, buoy_analysis, pressure_analysis)
    ...
    def _estimate_conditions(
        ...
        has_groundswell = any(
            swell.get('period', 0) > 12 or
            ('period_str' in swell and '12' in swell['period_str'])
            for swell in swells
        )
    ...
    def _estimate_timing(self, swells: List[Dict[str, Any]]) -> str:
        ...
        building_count = sum(
            1 for swell in swells
            if swell.get('trend', '') in ['increasing_strong', 'increasing_moderate']
        )

        # Check for arrivals
        future_arrivals = [
            swell for swell in swells
            if 'arrival' in swell and swell.get('confidence', 0) > 0.6
        ]
                ...
                if direction_deg is not None:
                    ...
                    for existing_dir in list(swell_map.keys()):
                        if self._directions_match(str(direction_deg), existing_dir):
                            ...
                            swell_map[existing_dir]['buoy_height'] = f"{trend.get('height_current')}m"
                            swell_map[existing_dir]['buoy_period'] = f"{trend.get('period_current')}s"
                            matched = True
                            break
    ...
    def _calculate_synthesis_confidence(
        ...
        high_impact_count = sum(
            1 for c in contradictions if c.get('impact') == 'high'
        )
        medium_impact_count = sum(
            1 for c in contradictions if c.get('impact') == 'medium'
        )
...
"""
        ...
        if swell_breakdown:
            ...
            for swell in swell_breakdown:
                narrative += f"- {swell['direction']} swell: {swell['height']} @ {swell['period']}\n"
