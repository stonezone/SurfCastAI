...
class ForecastValidator:
    ...
    async def validate_forecast(
        ...
        self.logger.info(f"Calculated metrics: MAE={metrics['mae']:.2f}, RMSE={metrics['rmse']:.2f}")
        ...
        for match in matches:
            ...
            height_error = abs(pred['height'] - actual['wave_height']) if pred.get('height') and actual.get('wave_height') else None
            period_error = abs(pred['period'] - actual['dominant_period']) if pred.get('period') and actual.get('dominant_period') else None
            ...
            category_match = (pred_category == actual_category) if pred_category and actual_category else None
    ...
    async def _get_forecast_data(
        ...
        with sqlite3.connect(self.database.db_path) as conn:
            ...
            forecast_time = datetime.fromisoformat(created_at) if isinstance(created_at, str) else datetime.fromtimestamp(created_at)
            ...
            for row in cursor.fetchall():
                predictions.append({
                    'id': row[0],
                    'shore': row[1],
                    'forecast_time': datetime.fromisoformat(row[2]) if isinstance(row[2], str) else datetime.fromtimestamp(row[2]),
                    'valid_time': datetime.fromisoformat(row[3]) if isinstance(row[3], str) else datetime.fromtimestamp(row[3]),
                    'height': row[4],
                    'period': row[5],
                    'direction': row[6],
                    'category': row[7],
        ...
        async with BuoyDataFetcher() as fetcher:
            for shore in shores:
                ...
                try:
                    ...
                    self.logger.error(f"Error fetching observations for {shore}: {e}", exc_info=True)
    ...
    def _categorize_height(self, height: Optional[float]) -> Optional[str]:
        """
