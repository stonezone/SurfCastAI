...
from ..utils.swell_propagation import HAWAII_LAT, HAWAII_LON, SwellPropagationCalculator
...
class StormInfo(BaseModel):
    ...
    central_pressure_mb: float | None = Field(
        None, ge=900.0, le=1100.0, description="Central pressure in mb"
    )
    fetch_nm: float | None = Field(None, gt=0, description="Fetch length in nautical miles")
    duration_hours: float | None = Field(None, gt=0, description="Storm duration in hours")
...
class StormDetector:
    ...
    def parse_pressure_analysis(self, analysis_text: str, timestamp: str) -> list[StormInfo]:
        """
        ...
        for section in sections:
            ...
            confidence = self._calculate_confidence(coords, wind_speed, pressure, fetch, duration)
    ...
    def _infer_from_region(self, text: str) -> tuple[float, float] | None:
    ...
    def _extract_fetch(self, text: str) -> float | None:
    ...
    def _generate_storm_id(self, coords: tuple[float, float], timestamp: str, counter: int) -> str:
    ...
    def summarise_upper_air(self, products: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in products or []:
            ...
            bucket = summary.setdefault(
                level, {"product_type": entry.get("product_type"), "products": []}
            )
    ...
    def estimate_missing_parameters(self, storm: StormInfo) -> StormInfo:
        ...
        if storm.fetch_nm is None:
            ...
            self.logger.debug(
                f"Estimated fetch: {storm.fetch_nm}nm from wind speed {storm.wind_speed_kt}kt"
            )
        ...
        if storm.duration_hours is None:
            ...
            self.logger.debug(
                f"Estimated duration: {storm.duration_hours}hrs from pressure {storm.central_pressure_mb}mb"
            )

        return storm

    def calculate_hawaii_arrivals(
        self, storms: list[StormInfo], propagation_calc: SwellPropagationCalculator | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for storm in storms:
            try:
                ...
                detection_time = datetime.fromisoformat(storm.detection_time.replace("Z", "+00:00"))
...
def example_usage():
    ...
    for arrival in arrivals:
        ...
        print(
            f"Estimated: {arrival['estimated_height_ft']}ft @ {arrival['estimated_period_seconds']:.1f}s"
        )
