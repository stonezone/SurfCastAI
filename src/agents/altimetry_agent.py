    ...
    async def collect(self, data_dir: Path) -> List[Dict[str, Any]]:
        ...
        for idx, url in enumerate(urls):
            ...
            if result.get('status') == 'success':
                ...
                metadata.append(result)
                if is_erddap_graph:
                    self.logger.warning(f"Failed to download from {source}, trying fallback if available")
    ...
    def _summarise_netcdf(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            with Dataset(file_path, mode='r') as dataset:
                summary['netcdf_dimensions'] = {name: len(dim) for name, dim in dataset.dimensions.items()}
