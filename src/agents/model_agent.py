...
class ModelAgent(BaseAgent):
    ...
    async def process_model_url(self, url: str, model_dir: Path) -> Dict[str, Any]:
        ...
        for resolved_url in self._expand_url_templates(url):
            try:
                ...
                self.logger.info(
                    "Processing %s model data for %s (%s)",
                    model_type,
                    location,
                    resolved_url
                )
                ...
                if resolved_url.endswith(('.png', '.jpg', '.gif')):
                    result = await self._process_model_image(resolved_url, model_dir, model_type, location)
                # Check for data files - handle both direct extensions and query string formats (ERDDAP)
                elif (resolved_url.endswith(('.json', '.txt', '.csv')) or 
                      '.csv?' in resolved_url or '.json?' in resolved_url or '.txt?' in resolved_url):
                    result = await self._process_model_data_file(resolved_url, model_dir, model_type, location)
                else:
                    result = await self._process_model_page(resolved_url, model_dir, model_type, location)
                ...
                last_error = str(e)
                self.logger.warning(f"Failed model URL %s: %s", resolved_url, e)
                continue
    ...
    def _expand_url_templates(self, url: str) -> List[str]:
        ...
        for date_value in candidates:
            expanded.append(url.replace("{date}", date_value))
        return expanded
        ...
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as fh:
            for raw_line in fh:
                ...
                if is_erddap and not units_row_found:
                    ...
                    if any(unit in line for unit in ['UTC', 'degrees_north', 'degrees_east', 'meters', 'm']):
    ...
    def _parse_erddap_records(self, records: List[Dict[str, str]], summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _parse_nomads_records(self, records: List[Dict[str, str]], summary: Dict[str, Any]) -> Dict[str, Any]:
