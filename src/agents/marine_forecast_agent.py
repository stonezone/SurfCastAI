...
class MarineForecastAgent(BaseAgent):
    ...
    async def collect(self, data_dir: Path) -> List[Dict[str, Any]]:
        ...
        forecast_urls = self.config.get_data_source_urls('marine_forecasts').get('marine_forecasts', [])
    ...
    async def process_forecast(self, url: str, forecast_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.create_metadata(
                name=f"marine_forecast_unknown",
                description=f"Error processing marine forecast",
                data_type='json',
                source_url=url,
                error=str(e)
            )
